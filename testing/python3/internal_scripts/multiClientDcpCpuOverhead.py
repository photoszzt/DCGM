...
# Either set this to None to use all gpus or a list of gpuIds like [0,1]
gpuIds = None


fieldIds = [1001, 1004, 1005, 1009, 1010,
            1011, 1012]  # DCGM-exporter default list
...
print("DCGM used %.f%% CPU with %d idle clients" %
      (idleClientsPct, numClients))
...
for i in range(numClients):
    watchers.append(FieldWatcher(
        clientHandles[i], gpuIds, fieldIds, watchInterval))
    watchers[-1].Watch()

    # Don't measure watch start-up overhead
    discard = getProcessPct(dcgmProcess)
