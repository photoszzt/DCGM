...
class NvSwitchCounterMonitor:
    ...
    def _InitHandles(self) -> None:
        ...
        self._nvSwitchLatencyFieldGroup = pydcgm.DcgmFieldGroup(
            self._dcgmHandle, name=fgName, fieldIds=self._nvSwitchLatencyFieldIds)

        fgName = "bandwidth0_mon_nvswitches" + self._pidPostfix
        self._nvSwitchBandwidth0FieldGroup = pydcgm.DcgmFieldGroup(
            self._dcgmHandle, name=fgName, fieldIds=self._nvSwitchBandwidth0FieldIds)

        fgName = "bandwidth1_mon_nvswitches" + self._pidPostfix
        self._nvSwitchBandwidth1FieldGroup = pydcgm.DcgmFieldGroup(
            self._dcgmHandle, name=fgName, fieldIds=self._nvSwitchBandwidth1FieldIds)
            ...
            for entityId in self._nvSwitchLatencyWatcher.values[entityGroupId]:
                ...
                for linkIdx in range(0, self.NVSWITCH_NUM_LINKS):
                    ...
                    if latencyFieldId in self._nvSwitchLatencyWatcher.values[entityGroupId][entityId]:
                        latencyLow = self._nvSwitchLatencyWatcher.values[
                            entityGroupId][entityId][latencyFieldId].values[-1].value
                        latencyFieldId += 1
                        latencyMed = self._nvSwitchLatencyWatcher.values[
                            entityGroupId][entityId][latencyFieldId].values[-1].value
                        latencyFieldId += 1
                        latencyHigh = self._nvSwitchLatencyWatcher.values[
                            entityGroupId][entityId][latencyFieldId].values[-1].value
                        latencyFieldId += 1
            ...
            for entityId in self._nvSwitchBandwidth0Watcher.values[entityGroupId]:
                ...
                for linkIdx in range(0, self.NVSWITCH_NUM_LINKS):
                    ...
                    if bandwidth0FieldId in self._nvSwitchBandwidth0Watcher.values[entityGroupId][entityId]:
                        ...
                        bandwidth0FieldId += 1
                        bandwidth1FieldId += 1
                        counter0Rx = self._nvSwitchBandwidth0Watcher.values[
                            entityGroupId][entityId][bandwidth0FieldId].values[-1].value
                        counter1Rx = self._nvSwitchBandwidth1Watcher.values[
                            entityGroupId][entityId][bandwidth1FieldId].values[-1].value
                        bandwidth0FieldId += 1
                        bandwidth1FieldId += 1
...
def main() -> None:
    ...
    print(("Using hostname %s and update interval as %d secs " %
          (hostname, counterMonitor._updateIntervalSecs)))
