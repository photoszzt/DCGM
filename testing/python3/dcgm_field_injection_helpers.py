...
def inject_nvml_value(handle, gpuId, fieldId, value, offset):
    ...
    if fieldType == dcgm_fields.DCGM_FT_INT64:
        ...
        ret = dcgm_agent_internal.dcgmInjectEntityFieldValueToNvml(
            handle, dcgm_fields.DCGM_FE_GPU, gpuId, field)
    else:
...
def inject_value(handle, entityId, fieldId, value, offset, verifyInsertion: bool=True,
    ...
    if fieldType == dcgm_fields.DCGM_FT_INT64:
        ret = inject_field_value_i64(
            handle, entityId, fieldId, value, offset, entityGroupId=entityType)
    ...
    if verifyInsertion:
        assert ret == dcgm_structs.DCGM_ST_OK, "Could not inject value %s in field id %s" % (
            value, fieldId)
    return ret
