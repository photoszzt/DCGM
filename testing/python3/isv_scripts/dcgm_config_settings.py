...
def agent_worker_function(handle, groupId, groupInfo, status_handle) -> None:
    ...
    while True:
        ...
        for x in range(0, groupInfo.count):
            ...
            print("Ecc  Mode   : %s" %
                  (convert_value_to_string(config_values[x].mEccMode)))
            print("Auto Boost  : %s" % (convert_value_to_string(
                config_values[x].mPerfState.autoBoost)))
            print("Sync Boost  : %s" % (convert_value_to_string(
                config_values[x].mPerfState.autoBoost)))
            print("Mem Clock   : %s" % (convert_value_to_string(
                config_values[x].mPerfState.minVPState.memClk)))
            print("SM  Clock   : %s" % (convert_value_to_string(
                config_values[x].mPerfState.minVPState.procClk)))
            print("Power Limit : %s" %
                  (convert_value_to_string(config_values[x].mPowerLimit.val)))
            print("Compute Mode: %s" %
                  (convert_value_to_string(config_values[x].mComputeMode)))
            print("\n")
...
if __name__ == "__main__":
    ...
    with RunDCGM('127.0.0.1', dcgm_structs.DCGM_OPERATION_MODE_MANUAL) as handle:
        ...
        thread = Thread(target=agent_worker_function, args=(
            handle, groupId, groupInfo, status_handle))
        thread.start()
        ...
        assert (
            ret == dcgm_structs.DCGM_ST_OK), "Failed to remove the test group, error: %s" % ret
