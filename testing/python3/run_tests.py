...
def kill_hostengine_if_needed() -> None:
    ...
    for i in range(0, 2):
        process_ids = test_utils.check_for_running_hostengine_and_log_details(
            True)
...
def run_tests() -> None:
    ...
    with test_utils.SubTest("Main", count=False, dvssc_log=False):
        ...
        if option_parser.options.use_running_hostengine:
            hostengineFn = test_utils.RunStandaloneHostEngine
        else:
            hostengineFn = test_utils.RunEmbeddedHostEngine

        with hostengineFn() as hostengine:
            if option_parser.options.use_running_hostengine:
                initFn = test_utils.RunClientInitShutdown
                handle = None
            else:
            ...
            with initFn(hostengine, handle=handle) as handle:
                ...
                if not test_utils.nvmlNotLoaded and dcgmGpuCount > 0:
        ...
        if run_compiled_flag:
            ...
            if len(test_content) == 0 and option_parser.options.filter_tests:
                logger.info(
                    "Filtered test is not be present in compiled file (--fast-run). Please run without --fast-run for filtered tests")
        else:
...
def print_test_info() -> None:
    ...
    for module in test_content:
        ...
        # all tests are in testDir. module there's no use in printing that
        module_name = module_name.replace("%s." % testDirWithDots, "", 1)
        for function in module[1]:
            ...
            if option_parser.options.verbose:
                ...
                function_doc = _test_info_split_verbose_first_newlines.sub(
                    "", function_doc)
                function_doc = _test_info_split_verbose_last_newlines.sub(
                    "", function_doc)
                print("%s.%s:\n%s\n" %
