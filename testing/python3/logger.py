...
log_dir = None
# Have we removed and recreated the log dir from previous runs?
have_recreated_log_dir = False
        ...
        def genfunc(fn: FunctionType):
            ...
            def tmpfn(*args, **kwargs):
                debug("Call %s(args: %s kwargs: %s)" % (fn.__name__, list(
                    zip(inspect.getfullargspec(fn).args, args)), kwargs), caller_depth=1)
                try:
                    res = fn(*args, **kwargs)
                    debug("Call %s returned: %s" %
                          (fn.__name__, res), caller_depth=1)
                    return res
                except Exception as e:
                    debug("Call %s raised: %s" %
                          (fn.__name__, e), caller_depth=1)
                    raise
            return tmpfn
...
def setup_environment() -> None:
    ...
    if option_parser.options.log_dir:
        ...
        curr_log_dir = os.path.normpath(os.path.join(
            option_parser.options.log_dir, default_log_dir))
    else:
    ...
    if not have_recreated_log_dir:
        ...
        try:
            ...
            pass
        # so that non-root tests could write to this directory
        ...
        with IndentBlock():
            try:
                version_file = open(os.path.join(
                    utils.script_dir, "data/version.txt"), "wr+")
...
def capture_dmesg() -> None:
    ...
    try:
        ...
        error(
            f"Failed to capture dmesg output (rc={e.returncode}): {e.stderr}")
        return
...
def close() -> None:
    ...
    if log_dir is not None and os.path.isdir(log_dir):
        try:
            zip = zipfile.ZipFile(log_archive_filename,
...
def run_with_coverage(fn) -> CoverageResults:
    ...
    coverage_trace = trace.Trace(
        trace=0, ignoredirs=[sys.prefix, sys.exec_prefix])
    try:
    ...
    def apply_coloring(level: int, line: str):
        ...
        for (what, color) in coloring:
            ...
            if coloring_match:
                ...
                line = "".join([groups[0], color, groups[1],
                               colorama.Fore.RESET, groups[2]])
        return line
