...
class RunValidateDcgm:
    ...
    def removeBinaries(self, prnt: bool) -> int:
        ...
        ret = util.executeBashCmd(
            "sudo rm -rf datacenter-gpu-manager*.deb", prnt)
    ...
    def downloadInstallers(self, dcgm_url, deb_url) -> int:
        ...
        fileName = wget.download(dcgm_url, out=None)
    ...
    def installDcgm(self):
        ...
        ret = util.executeBashCmd(
            "sudo dpkg -i datacenter-gpu-manager-dcp-nda-only_1.6.4_amd64.deb", True)
        if ret[0] != 0:
    ...
    def validateAccuracyForUtilForUtil(self, dcgmCsvFile: str, dcgmProfTesterCsvFile: str, gpu_index: int, metrics: int) -> int:
        ...
        with open(dcgmProfTesterCsvFile, 'r') as f2:
            ...
            data = pandas.read_csv(dcgmCsvFile, skiprows=[0], names=colnames)
            dcgm_list = data[dcgm_col].tolist()
            i = self.getClosestIndex(
                dcgm_list, prof_list, metrics, dcgm_init_val)
            dcgm_delay = 0
            try:
            ...
            print('Total comparisons: ' + str(tot_comp) +
...
def main(cmdArgs: Namespace) -> None:
    ...
    for i in range(0, len(gpu_list)):
        ...
        dcgmproftester_file = 'dcgmProfTester_{0}_gpu{1}.csv'.format(
            metrics, gpu_list[i])
        ret = ro.validateAccuracy(
            dcgm_file, dcgmproftester_file, int(gpu_list[i]), metrics)
        if ret == 0:
            print("\n&&&& PASSED GPU_" +
                  str(gpu_list[i]) + "_metric_validation_test")
    ...
    print("Memory in Between of test run of all GPU's under test: " +
          str(mem_in_between))
    print("Memory in end of test run of all GPU's under test: " +
          str(mem_in_end) + "\n")

    for i in range(0, len(gpu_list)):
        print("\n&&&& RUNNING GPU_" +
              str(gpu_list[i]) + "_memory_validation_test")
