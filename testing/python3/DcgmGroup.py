...
class DcgmGroupConfig:
    ...
    def Enforce(self) -> None:
        ...
        ret = dcgm_structs.DCGM_ST_OK
        try:
            ret = dcgm_agent.dcgmConfigEnforce(
                self._dcgmHandle.handle, self._groupId, status.handle)
        except dcgm_structs.DCGMError as e:
            pass
...
class DcgmGroupHealth:
    ...
    def Get(self):
        systems = dcgm_agent.dcgmHealthGet(
            self._dcgmHandle.handle, self._groupId)
        return systems
...
class DcgmGroupPolicy:
    ...
    def Set(self, policy: c_dcgmPolicy_v1, statusHandle=None) -> None:
        ...
        dcgm_agent.dcgmPolicySet(
            self._dcgmHandle.handle, self._groupId, policy, statusHandle)
        self._policy = policy
...
class DcgmGroupAction:
    ...
    def Validate(self, validate):
        ...
        ret = dcgm_agent.dcgmActionValidate_v2(
            self._dcgmHandle.handle, runDiagInfo)
        return ret
    ...
    def RunDiagnostic(self, diagLevel):
        ret = dcgm_agent.dcgmRunDiagnostic(
            self._dcgmHandle.handle, self._groupId, diagLevel)
        return ret
...
class DcgmGroup:
    ...
    def __init__(self, dcgmHandle, groupId=None, groupName: Self | None=None, groupType: int=dcgm_structs.DCGM_GROUP_EMPTY) -> None:
        ...
        if groupId is not None:
            ...
            self._groupId = dcgm_agent.dcgmGroupCreate(
                self._dcgmHandle.handle, groupType, groupName)
        ...
        self.discovery = DcgmGroupDiscovery(
            self._dcgmHandle, self._groupId, self)
        ...
        self.profiling = DcgmGroupProfiling(
            self._dcgmHandle, self._groupId, self)
        with DcgmGroup.groupLock:
            if dcgmHandle.FromDecorator() and (self.name is not None):
                if dcgmHandle not in DcgmGroup.groups:
                    DcgmGroup.groups[dcgmHandle] = {}
                    add = True
                elif self.name not in DcgmGroup.groups[dcgmHandle]:
                    add = True
                else:
                    add = False

                if add:
                    DcgmGroup.groups[dcgmHandle][self.name] = self
    ...
    def Delete(self) -> None:
        ...
        if self._groupId is not None and not self._IsGroupIdStatic():
            ...
            with DcgmGroup.groupLock:
                if (
                    self._dcgmHandle in DcgmGroup.groups
                    and self.name in DcgmGroup.groups[self._dcgmHandle]
                ):
                    del DcgmGroup.groups[self._dcgmHandle][self.name]
    ...
    def clear(cls, dcgmHandle=None) -> None:
        with cls.groupLock:
            if dcgmHandle is None:
                ...
                if dcgmHandle in cls.groups:
                    del cls.groups[dcgmHandle]
    ...
    def AddGpu(self, gpuId) -> None:
        ...
        ret = dcgm_agent.dcgmGroupAddDevice(
            self._dcgmHandle.handle, self._groupId, gpuId)
        dcgm_structs._dcgmCheckReturn(ret)
    ...
    def AddEntity(self, entityGroupId, entityId) -> None:
        ...
        ret = dcgm_agent.dcgmGroupAddEntity(
            self._dcgmHandle.handle, self._groupId, entityGroupId, entityId)
        dcgm_structs._dcgmCheckReturn(ret)
    ...
    def RemoveEntity(self, entityGroupId, entityId) -> None:
        ...
        ret = dcgm_agent.dcgmGroupRemoveEntity(
            self._dcgmHandle.handle, self._groupId, entityGroupId, entityId)
        dcgm_structs._dcgmCheckReturn(ret)
