    ...
    def __init__(self, hostList=None, hostEngineAddress=None, testName=None, parameters=[],
        ...
        if hostList:
            for i, host in enumerate(hostList):
                ...
                self.runMnDiagInfo.hostList[i][:] = b'\0' * \
                    dcgm_structs.DCGM_MAX_STR_LENGTH
        ...
        if parameters:
            for i, param in enumerate(parameters):
                ...
                self.runMnDiagInfo.testParms[i][:] = b'\0' * \
                    dcgm_structs.DCGM_MAX_TEST_PARMS_LEN_V2
                self.runMnDiagInfo.testParms[i][:len(
