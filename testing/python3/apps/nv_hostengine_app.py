...
class NvHostEngineApp(app_runner.AppRunner):
    ...
    def __init__(self, args=None, profile_dir=None, heEnv=None, pid_dir=None) -> None:
        ...
        if pid_dir is not None:
            ...
            self._pidFilename = os.path.join(
                logger.log_dir, 'nv-hostengine.pid')

        if option_parser.options.profile:
            self._check_valgrind_installed()
            self.output_dir = self._create_output_dir(
                option_parser.options.profile, profile_dir)

            args, path = self._create_profile_command(
                args, path, option_parser.options.profile)
        ...
        if not test_utils.noLogging and not option_parser.options.profile:
            ...
            logger.nvvs_trace_log_filename = os.path.join(
    ...
    def _create_profile_command(self, args, path: str, valgrind_tool):
        ...
        if valgrind_tool not in self.supported_profile_tools:
            raise Exception(
                '%s is not a supported tool for profiling' % valgrind_tool)
        ...
        common_args = [
            ...
            '--log-file=%s' % os.path.join(self.output_dir,
                                           '%s.log.%%p' % valgrind_tool)
        ]
        tool_args = []
        tool_log_file = os.path.join(
            self.output_dir, valgrind_tool + '.out.%p')
    ...
    def start(self, timeout: float=default_timeout) -> None:
        ...
        if self._pid is None:
            ...
            retValue = super(NvHostEngineApp, self).poll()
            if retValue is None:
            ...
            logger.error(
                "Could not start nvhostengine. Output from the failed launch (if any) follows.")
