...
class RmCall:
    ...
    regexpRmCallReleaseReturn = re.compile(
        r"^([0-9a-f]*) ([0-9a-f]*) ## 0x([0-9a-f]*)$")
    regexpRmCallDebug = re.compile(
        r"^dcgmRmCall\(([a-zA-Z0-9_.]* [0-9a-f]*), (\w*), \.\.\.\)$")
    regexpRmCallDebugReturn = re.compile(
        r"^dcgmRmCall\(([a-zA-Z0-9_.]* [0-9a-f]*), (\w*), \.\.\.\) returned 0x([0-9a-f]*)$")
    regexpFilePath = re.compile(r".*dmal/rm/.*\.c$")
    regexpRmCallSrc = re.compile(
    ...
    def __init__(self, debugLines, i, dcgmParent) -> None:
        ...
        if not line1.isInTheSamePlace(line2):
            verbose_debug(
                "RmCall: Failed because they are not in the same line")
            raise ValueError
        if not RmCall.regexpFilePath.match(line1.srcfilename):
            verbose_debug(
                "RmCall: Failed because they are not in the correct file in dmal/rm/rm_*")
            raise ValueError
        ...
        if not match1 or not match2:
            ...
            self.rmCallName = 1
            verbose_debug(
                "RmCall: Failed match regexpRmCallRelease* but still in trying other regexps")
        ...
        # so that stats from multiple runs could be appended
        self.times = [self.time]
        self.dcgmParent = dcgmParent if dcgmParent and dcgmParent.isParentOfRmCall(
            self) else None
...
class NvmlCall:
    ...
    regexpNvmlCall = re.compile(
        r"^Entering (dcgm[a-zA-Z0-9_]*)(\(.*\)) *(\(.*\))$")
    ...
    def __init__(self, debugLines, i):
        ...
        if not NvmlCall.regexpFilePath.match(line1.srcfilename):
            self.istsapi = 0
            verbose_debug(
                "NvmlCall: Wrong file name also matching non tsapi regexp")
            if not NvmlCall.regexpFilePathNonTsapi.match(line1.srcfilename):
                verbose_debug(
                    "NvmlCall: Wrong file also doesn't match non tsapi regexp")
                raise ValueError
        ...
        if not match1:
            ...
            self.dcgmCallName = 0
            verbose_debug(
                "NvmlCall: Failed match regexpNvmlCall but need to also try regexpNvmlIntRelCall")
        ...
        for j in range(i + 1, len(debugLines)):
            ...
            verbose_debug("NvmlCall: Matched the end line %d %s" %
                          (j, str(line2)))

            # TODO match device
            self.src = line1
            self.srcEnd = line2
            self.function = match1.group(
                1) if self.dcgmCallName else "NVML INT " + self.src.srcStr()
...
class PerformanceStats(object):
    def __init__(self, input_fname) -> None:
        ...
        if len(lines) > 0:
            self.time_total = lines[-1].timestamp - \
                lines[0].timestamp + epsilon
        else:
            self.time_total = -1
    ...
    def combine_stat(self, perf_stat) -> None:
        ...
        if (len(perf_stat.stats) != len(self.stats)):
            ...
            if not test_utils.noLogging:
                fname_log_perf_stat = os.path.relpath(
                    os.path.join(logger.log_dir, "log_perf_stat.txt"))
                fname_log_self_stat = os.path.relpath(
                    os.path.join(logger.log_dir, "log_self_stat.txt"))
        ...
        for i in range(len(self.stats)):
            ...
            assert stat1.is_simillar(
                stat2), "stat %d: %s doesn't match %s and can't be combined" % (i, stat1, stat2)
