...
def helper_dcgm_sysmon_cpu_hierarchy(hierarchy) -> None:
    ...
    for i in range(hierarchy.numCpus):
        ...
        for cpu_index in range(pydcgm.dcgm_structs.DCGM_MAX_NUM_CPUS):
            ...
            bitmask_value = int(
                hierarchy.cpus[i].ownedCores.bitmask[bitmask_index]) & bitmask_mask
            ...
            cpu_owns_core = bool(pydcgm.dcgm_agent.dcgmCpuHierarchyCpuOwnsCore(
                cpu_index, hierarchy.cpus[node_id].ownedCores))
...
def test_sysmon_reading_injected_values(handle, cpuIds, coreIds) -> None:
    ...
    for value in values:
        ...
        assert value.status == dcgm_structs.DCGM_ST_OK, "Couldn't read field %d: '%s'" % (
            value.fieldId, str(dcgm_structs.DCGMError(value.status)))

        if value.fieldId == dcgm_fields.DCGM_FI_DEV_CPU_CLOCK_CURRENT:
            assert value.value.i64 == injection_info[index][1], "Expected %d but read %d" % (
                injection_info[index][1], value.value.i64)
        else:
...
def test_dcgm_sysmon_fields_with_dcgmreader(handle) -> None:
    ...
    cpuIds = dcgm_agent.dcgmGetEntityGroupEntities(
        handle, dcgm_fields.DCGM_FE_CPU, 0)
    cpuEntities = []
    for cpu in cpuIds:
        entityPair = dcgm_structs.c_dcgmGroupEntityPair_t()
    ...
    for i in range(5):
        ...
        cpuLatest = dr.GetLatestEntityValuesAsFieldIdDict()[
            dcgm_fields.DCGM_FE_CPU]
        ...
        for cpuId in cpuIds:
            if len(cpuLatest[cpuId]) != len(fieldIds):
                ...
                errmsg = "i=%d, cpuId %d, len %d != %d" % (
                    i, cpuId, len(cpuLatest[cpuId]), len(fieldIds))
