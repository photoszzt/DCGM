...
def helper_test_dcgm_error_get_priority(handle, gpuIds) -> None:
    ...
    prio = dcgm_errors.dcgmErrorGetPriorityByCode(
        dcgm_errors.DCGM_FR_ERROR_SENTINEL)
...
def helper_test_dcgm_error_get_msg(handle, gpuIds) -> None:
    ...
    assert msg == dcgm_errors.DCGM_FR_NVLINK_CRC_ERROR_THRESHOLD_MSG, \
        "Expected '%s' as msg, but found '%s'" % (
            dcgm_errors.DCGM_FR_NVLINK_CRC_ERROR_THRESHOLD_MSG, msg)

    msg = dcgm_errors.dcgmErrorGetFormatMsgByCode(
        dcgm_errors.DCGM_FR_DEVICE_COUNT_MISMATCH)
    assert msg == dcgm_errors.DCGM_FR_DEVICE_COUNT_MISMATCH_MSG, \
        "Expected '%s' as msg, but found '%s'" % (
            dcgm_errors.DCGM_FR_DEVICE_COUNT_MISMATCH_MSG, msg)
