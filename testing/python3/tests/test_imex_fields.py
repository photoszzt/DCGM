...
def helper_imex_fields_basic_retrieval(handle, gpuIds) -> None:
    ...
    groupId = dcgm_agent.dcgmGroupCreate(
        handle, dcgm_structs.DCGM_GROUP_DEFAULT, "imex_test_group")
    ...
    try:
        ...
        logger.info(
            f"Domain field found: {domain_found}, Daemon field found: {daemon_found}")
...
def helper_imex_domain_status_values(handle, gpuIds) -> None:
    ...
    try:
        ...
        dcgm_agent.dcgmWatchFields(
            handle, groupId, fieldGroupId, updateFreq, 3600.0, 0)
        ...
        valid_statuses = ["UP", "DOWN", "DEGRADED",
                          "NOT_INSTALLED", "NOT_CONFIGURED", "UNAVAILABLE"]
...
def helper_imex_daemon_status_values(handle, gpuIds) -> None:
    ...
    try:
        ...
        for i, value in enumerate(values):
            logger.info(
                f"Value {i}: field_id={value.fieldId}, status={value.status}")

        # Check if we got a valid daemon status field
        valid_values = [v for v in values if v.fieldId ==
                        dcgm_fields.DCGM_FI_IMEX_DAEMON_STATUS]

        if len(valid_values) == 0:
            logger.info(
                "IMEX daemon status not available in this environment - skipping test")
            return
        ...
        # Validate that the returned status is one of the expected values
        # 0-7: normal states, -1,-2,-3: error states
        valid_range = list(range(0, 8)) + [-1, -2, -3]
...
def helper_imex_fields_consistency(handle, gpuIds) -> None:
    ...
    groupId = dcgm_agent.dcgmGroupCreate(
        handle, dcgm_structs.DCGM_GROUP_DEFAULT, "imex_consistency_group")
...
def helper_imex_fields_update_frequency(handle, gpuIds) -> None:
    ...
    try:
        ...
        domain_samples = [v for v in values if v.fieldId ==
                          dcgm_fields.DCGM_FI_IMEX_DOMAIN_STATUS]
