    ...
    for ci in ciIds:
        for data in nonGlobalData:
            ...
            ret = dcgm_agent_internal.dcgmInjectEntityFieldValue(
                handle, dcgm_fields.DCGM_FE_GPU_CI, ci, field)
            assert (ret == dcgm_structs.DCGM_ST_OK)
        ...
        for entityId in latest[entityGroupId]:
            ...
            for fieldId in latest[entityGroupId][entityId]:
                ...
                for value in latest[entityGroupId][entityId][fieldId]:
                    ...
                    assert (fieldValues[fieldIds.index(
                        fieldId2)] == value.value)
                    foundValues += 1
                    break
...
def test_dcgm_reader_wildcard_gi(handle, gpuIds, instanceIds, ciIds) -> None:
    ...
    assert (ciIds[0] / gpuIds[1]) == dcgm_structs.DCGM_MAX_COMPUTE_INSTANCES_PER_GPU, "CIs Expected value %d, observed value: %d" % (
...
def test_dcgm_reader_latest_mig_ci_fields_by_id(handle, gpuIds, instanceIds, ciIds) -> None:
    ...
    helper_dcgm_reader_latest_mig_ci_fields(
...
def test_dcgm_reader_all_mig_ci_fields_by_id(handle, gpuIds, instanceIds, ciIds) -> None:
    ...
    helper_dcgm_reader_all_mig_ci_fields(
        handle, gpuIds, instanceIds, ciIds, True)
