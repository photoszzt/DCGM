...
def _plotFinalValueOrderedBarChart(points, title, ylabel, filenameBase, topValCount=20) -> None:
    ...
    if logger.log_dir is None:
        logger.info(
            'not generating ordered bar chart since logging is disabled')
        return

    width = 4

    topSortedPts = sorted(points, key=operator.itemgetter(1), reverse=True)[
        :topValCount]
    ...
    plt.savefig(figName)
    plt.close()
    logger.info('ordered bar chart for %s saved in %s' %
...
def _generate_metadata_line_charts(metadataTSeries: MetadataTimeseries, ylabel, title) -> None:
    ...
    filename = '%s-%s.png' % ('-'.join(title.split()),
                              test_dcgm_standalone_perf_bounded.__name__)
    figName = os.path.join(logger.log_dir, filename)
    plt.savefig(figName)
    plt.close()
    ...
    while (datetime.datetime.now() - startTime).total_seconds() < BOUNDED_TEST_DURATION:
        ...
        for fieldGroup in fieldGroups:
            memUsageTS.fieldGroupVals[int(fieldGroup.fieldGroupId.value)].append(
                system.introspect.memory.GetForFieldGroup(fieldGroup).aggregateInfo.bytesUsed)
        ...
        execTimeTS.allFieldsVals.append(
            system.introspect.execTime.GetForAllFields().aggregateInfo.totalEverUpdateUsec)
        ...
        for fieldGroup in fieldGroups:
            execTimeTS.fieldGroupVals[int(fieldGroup.fieldGroupId.value)].append(
                system.introspect.execTime.GetForFieldGroup(fieldGroup).aggregateInfo.totalEverUpdateUsec)
        ...
        execTimeAvgTS.allFieldsVals.append(
            system.introspect.execTime.GetForAllFields().aggregateInfo.recentUpdateUsec)
        ...
        for fieldGroup in fieldGroups:
            execTimeAvgTS.fieldGroupVals[int(fieldGroup.fieldGroupId.value)].append(
                system.introspect.execTime.GetForFieldGroup(fieldGroup).aggregateInfo.recentUpdateUsec)
...
def test_dcgm_standalone_perf_bounded(handle) -> None:
    ...
    test_utils.run_subtest(
        _test_cpuutil_bounded_flatlines_hostengine, cpuUtilTS)
    ...
    _generate_metadata_line_charts(
        memUsageTS, ylabel='bytes', title='Bytes Used')
    _generate_metadata_line_charts(
        execTimeTS, ylabel='usec', title='Execution Time')
    _generate_metadata_line_charts(
        execTimeAvgTS, ylabel='usec', title='Recent Exec Time')
...
def _generate_cpu_line_charts(cpuUtilTS: CpuTimeseries) -> None:
    ...
    if logger.log_dir is None:
        logger.info(
            'Not generating CPU utilization graphs since logging is disabled')
        return
    ...
    legendProxies = []
    for poly in polys:
        legendProxies.append(plt.Rectangle(
            (0, 0), 1, 1, fc=poly.get_facecolor()[0]))
    ...
    filename = '%s-%s.png' % ('CPU-Util',
                              test_dcgm_standalone_perf_bounded.__name__)
    figName = os.path.join(logger.log_dir, filename)
    plt.savefig(figName)
    plt.close()
    logger.info('%s figure saved in %s' %
