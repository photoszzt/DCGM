...
def helper_test_denylist_briefly() -> None:
    ...
    try:
        ...
        assert False, "Exception thrown when running the denylist app: '%s'" % str(
            e)
...
def helper_test_denylist_checks(handle, gpuIds) -> None:
    ...
    for gpuObj in denylist_recommendations.g_gpus:
        if gpuObj.GetEntityId() == gpuIds[0]:
            ...
            ) == False, "Injected error didn't trigger a failure on GPU %d" % gpuIds[0]
        else:
            assert gpuObj.IsHealthy(), "GPU %d reported unhealthy despite not having an inserted error: '%s'" % (
    ...
    for gpuObj in denylist_recommendations.g_gpus:
        if gpuObj.GetEntityId() == gpuIds[0]:
            assert gpuObj.IsHealthy(), "Injected error wasn't ignored for GPU %d: %s" % (
                gpuIds[0], gpuObj.WhyUnhealthy())
        else:
            assert gpuObj.IsHealthy(), "GPU %d reported unhealthy despite not having an inserted error: '%s'" % (
                gpuIds[0], gpuObj.WhyUnhealthy())
