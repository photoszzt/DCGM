...
def _summarize_tests() -> None:
    ...
    if tests_count > 0.0:
        tests_completed_ratio = float(
            tests_ok_count) / (float(tests_count) - (float(tests_fail_count / 2)))
    logger.info("Score:  %.2f" % (100.0 * tests_completed_ratio))
    logger.info("==================================\n\n")

    warnings_count = logger.messages_level_counts[logger.WARNING]
    if warnings_count > 0:
        logger.warning("Framework encountered %d warning%s" %
                       (warnings_count, utils.plural_s(warnings_count)))
    ...
    if tests_ok_count < tests_count:
        ...
        logger.info(" * Also attach %s file (it already contains nvml trace logs, nvidia-bug report and stdout)" %
...
def _run_burn_in_tests() -> None:
    ...
    if os.path.exists(file_name):
        ...
        if "__DCGM_DBG_FILE" in env:
            del env["__DCGM_DBG_FILE"]
        if "__NVML_DBG_FILE" in env:
            del env["__NVML_DBG_FILE"]

        if option_parser.options.dvssc_testing:
            burn = Popen([sys.executable, file_name, "-t", "3",
                         "--dvssc-testing"], stdout=None, stderr=None, env=env)
        else:
            burn = Popen([sys.executable, file_name, "-t", "3"],
...
class TestFrameworkSetup(object):
    def __enter__(self) -> None:
        ...
        if utils.is_mps_server_running():
            print(
                'DCGM Testing framework is not interoperable with MPS server. Please disable MPS server.')
            sys.exit(1)
        ...
        if not test_utils.verify_dcgmi_executible_visible_for_all_users():
            ...
            print(
                'See the logs to understand which part of the path lacks the read+execute permissions')
            print(
                'Either run `chmod o+rx <directory>` or move the DCGM Testing framework to another location')
            sys.exit(1)
        ...
        if not test_utils.is_framework_compatible():
            logger.fatal(
                "The test framework and dcgm versions are incompatible. Exiting Test Framework.")
            sys.exit(1)
        ...
        if utils.is_64bit():
            ...
            if not (platform.machine() == "aarch64"):
                if not os.path.exists(os.path.join(utils.script_dir, "apps/amd64")):
                    print(
                        "Testing package is missing 64bit binaries, are you sure you're using package of correct architecture?")
                    sys.exit(1)
        else:
            if not os.path.exists(os.path.join(utils.script_dir, "apps/x86")):
                print(
                    "Testing package is missing 32bit binaries, are you sure you're using package of correct architecture?")
                sys.exit(1)
        ...
        if utils.is_windows():
            if os.name == "nt" and "32 bit" in sys.version and platform.machine() == "AMD64":
                print(
                    "Running Python 32-bit on a 64-bit OS is not supported. Please install Python 64-bit")
                sys.exit(1)
    ...
    with TestFrameworkSetup():
        if not option_parser.options.no_env_check:
            if not test_utils.is_test_environment_sane():
                logger.warning(
                    "The test environment does not seem to be healthy, test framework cannot continue.")
                sys.exit(1)
        ...
        try:
            if not option_parser.options.no_process_check:
                ...
                logger.warning(
                    "Not checking for other processes using the GPU(s), test failures may occur.")
        except FileNotFoundError as e:
            logger.warning(
                "Skipping checking GPUs for other processes: nvidia-smi is not found.")

        if option_parser.options.test_info:
            print_test_info()
            return
        if option_parser.options.capture_nvml_environment_to is not None:
            test_utils.try_capture_nvml_env(
                option_parser.options.capture_nvml_environment_to)
            return
