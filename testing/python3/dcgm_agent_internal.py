...
def dcgmGetLatestValuesForFields(dcgmHandle: str, gpuId, fieldIds) -> Array[c_dcgmFieldValue_v1]:
    ...
    ret = fn(dcgmHandle, c_int(gpuId), id_values,
...
def dcgmGetMultipleValuesForField(dcgmHandle, gpuId, fieldId, maxCount, startTs, endTs, order):
    ...
    ret = fn(dcgmHandle, c_int(gpuId), c_uint(fieldId), byref(localMaxCount),
...
def dcgmInjectNvmlDevice(dcgmHandle: str, gpuId, key, extraKeys, extraKeyCount, injectNvmlRet):
    ...
    if extraKeys is None:
        ret = fn(dcgmHandle, c_uint(gpuId), key, None,
                 c_uint(extraKeyCount), byref(injectNvmlRet))
    else:
        ret = fn(dcgmHandle, c_uint(gpuId), key, byref(extraKeys),
                 c_uint(extraKeyCount), byref(injectNvmlRet))
    _dcgmIntCheckReturn(ret)
    return ret
...
def dcgmInjectNvmlDeviceForFollowingCalls(dcgmHandle, gpuId, key, extraKeys, extraKeyCount, injectNvmlRets, retCount):
    ...
    if extraKeys is None:
        ret = fn(dcgmHandle, c_uint(gpuId), key, None, c_uint(
            extraKeyCount), byref(injectNvmlRets), retCount)
    else:
        ret = fn(dcgmHandle, c_uint(gpuId), key, byref(extraKeys),
                 c_uint(extraKeyCount), byref(injectNvmlRets), retCount)
    _dcgmIntCheckReturn(ret)
    return ret
...
def dcgmResetNvmlInjectFuncCallCount(dcgmHandle):
    fn = dcgm_structs._dcgmGetFunctionPointer(
        "dcgmResetNvmlInjectFuncCallCount")
    ret = fn(dcgmHandle)
    _dcgmIntCheckReturn(ret)
    return ret
...
@dcgm_agent.ensure_byte_strings()
def dcgmInjectedNvmlDeviceReset(dcgmHandle, gpuId):
    fn = dcgm_structs._dcgmGetFunctionPointer("dcgmInjectedNvmlDeviceReset")
    ret = fn(dcgmHandle, c_uint(gpuId))
    _dcgmIntCheckReturn(ret)
    return ret
...
def dcgmGetFieldValuesSince(dcgmHandle, groupId, sinceTimestamp, fieldIds, enumCB, userData) -> int:
    ...
    ret = fn(dcgmHandle, groupId, c_int64(sinceTimestamp), c_fieldIds, c_int32(
        len(fieldIds)), byref(c_nextSinceTimestamp), enumCB, py_object(userData))
    _dcgmIntCheckReturn(ret)
...
@dcgm_agent.ensure_byte_strings()
def dcgmEmptyCache(dcgmHandle):
    fn = dcgm_structs._dcgmGetFunctionPointer("dcgmEmptyCache")
    ret = fn(dcgmHandle)
    _dcgmIntCheckReturn(ret)
    return ret
...
def dcgmMarkModulesReloadable(dcgmHandle: str, moduleMask) -> int:
    ...
    info.moduleMask = moduleMask
    ret = fn(dcgmHandle, byref(info))
    # info.moduleMask now contains a bitmask of modules that were NOT marked
    # reloadable.
    _dcgmIntCheckReturn(ret)

    return int(info.moduleMask)
