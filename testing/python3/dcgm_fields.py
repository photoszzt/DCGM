...
# GPU bind/unbind event notification. Values: SystemReinitializing=1, SystemReinitializationCompleted=2, recommended watch frequency: 1 second
...
# Result of the Input Energy Delayed Product power (EDPp) test (a.k.a. the pulse test)
...
# GUID string of the rack containing the GPU (e.g. xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)
...
# Index within the compute slots in the rack containing the GPU (does not include switches)
...
# The ratio of cycles the IMMA tensor pipe is active (off the peak sustained elapsed cycles)
DCGM_FI_PROF_PIPE_TENSOR_IMMA_ACTIVE = 1013
# The ratio of cycles the HMMA tensor pipe is active (off the peak sustained elapsed cycles)
...
# IMEX domain status (UP, DOWN, DEGRADED, NOT_INSTALLED, NOT_CONFIGURED, UNAVAILABLE)
DCGM_FI_IMEX_DOMAIN_STATUS = 1502
# IMEX daemon status (0-7: INITIALIZING through UNAVAILABLE, -1: NOT_INSTALLED, -2: NOT_CONFIGURED, -3: COMMAND_ERROR)
